...
function getProductActClubPrice(proIds, callback) {
	if(proIds=="") return ;
	$.ajax({
		url: window.jxdomain.act+"/price/clubAndPromoPrice?ids="+proIds+"&callback=" + callback,
        dataType : "jsonp",
        jsonp : callback,
	})
}
...
function getProductActAdDesWidthJsonp(proIds, callback) {
	...
	var act_request_url ="http://brand.jiuxian.com/selectProActInfoByGoodids.htm"+ "?ids=" + proIds + "&callback=" + callback;
	jQuery.ajax({
...
function getProductActPriceWidthJsonp(proIds, callbackName) {

	$.ajax({
		url: window.jxdomain.act+"/price/jsonp/currentPrice?ids="+proIds+"&callback=" + callbackName,
        dataType : "jsonp",
        jsonp : callbackName,
	})
}
