		...
		return this.each(function(){
			...
			$nextBtn.bind("click",function(){
				if(!isAnimate)
				{
					...
					if(currentPage==sliderLength)
					{
						$bigUl.stop(true,true).animate({"marginLeft":-currentPage*sliderWidth+"px"},option.speed,function(){
							$bigUl.css({"margin-left":"0px"});	
						});
						currentPage = 0;
						$smallUl.find("em:first").addClass("on").siblings().removeClass("on");				
					}else{
						$bigUl.stop(true,true).animate({"marginLeft":-currentPage*sliderWidth+"px"},option.speed);
						$smallUl.find("em").eq(currentPage).addClass("on").siblings().removeClass("on");
						}
				}else
				{					
					return false;						
			...
			$prevBtn.bind("click",function(){
				if(!isAnimate)
				{
					...
					{						
						...
						$smallUl.find("em:last").addClass("on").siblings().removeClass("on");	
			...
			/*数字按钮事件*/
			$smallUl.find("em").bind(option.trigger,function(){				
				if(!isAnimate)
				{
					var index = $smallUl.find("em").index($(this));
					currentPage = index;
					$bigUl.stop(true,true).animate({"marginLeft":-currentPage*sliderWidth+"px"},option.speed);
					$smallUl.find("em").eq(currentPage).addClass("on").siblings().removeClass("on");				
